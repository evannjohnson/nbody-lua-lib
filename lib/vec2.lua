...
-- function vec:setLength(mag)
-- 	local prevMag = self:Length()
-- 	self.x = self.x * (mag / prevMag)
-- 	self.y = self.y * (mag / prevMag)
-- end

function methods.rotate(self, theta)
    local cos = math.cos(theta)
    local sin = math.sin(theta)
...
function methods.setLength(self, mag)
    local prevMag = self:length()
    self[1] = self[1] * (mag / prevMag)
    self[2] = self[2] * (mag / prevMag)
    return self
end
